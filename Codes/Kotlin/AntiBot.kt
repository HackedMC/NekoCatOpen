...
@ModuleInfo(name = "AntiBot", description = "Fucked Matrix AntiBot!",category = ModuleCategory.MISC)
    ...
    fun isBot(entity: EntityLivingBase): Boolean {
        ...
        if (tabValue.get()) {
            ...
            for (networkPlayerInfo in mc.netHandler.playerInfoMap) {
                ...
                if (targetName != null) {
                    if (if (equals) targetName == networkName else networkName?.let { targetName.contains(it) } == true) {
                        return false
                    }
                }
            }
...
//                alert("NOCLIP[${cb.size}] ${entity.displayName.unformattedText} ${entity.posX} ${entity.posY} ${entity.posZ}")
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if(packet is S18PacketEntityTeleport) {
            ...
            if(NekoCat.combatManager.inCombat && !hasRemovedEntities.contains(packet.entityID)) {
