...
import net.ccbluex.liquidbounce.NekoCat
...
import net.ccbluex.liquidbounce.utils.MovementUtils2
...
@ModuleInfo(name = "BestAntiVoid", description = "Prevents you from falling into the void.",category = ModuleCategory.PLAYER)
class BestAntiVoid : Module() {
    ...
    open fun isInVoid(): Boolean {
        for (i in 0..128) {
            if (MovementUtils2.isOnGround(i.toDouble())) {
                return false
            }
        }
        return true
    ...
    fun onPacket(e: PacketEvent) {
        if (!NekoCat.moduleManager.get(Fly::class.java)!!.state && !NekoCat.moduleManager.get(Flight::class.java)!!.state && !NekoCat.moduleManager.get(Scaffold::class.java)!!.state) {
